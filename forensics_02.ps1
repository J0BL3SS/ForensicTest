...
$global:ScanKeywords = @{
    ...
    # NEW KEY ADDED HERE FOR STEP 46
    TransientSpace = @("update","temp","cache","install","setup")
}
...
function Scan-Step1 {
    $step = New-ScanStep 1 "PowerShell & CMD History Forensics" "Analyzes PSReadLine history for suspicious commands." "b-orange"

    # Clean and filter patterns once outside the loop
    $patterns = @($global:ScanKeywords.CmdHistory | ForEach-Object { $_.ToString().Trim() } | Where-Object { $_ })
    if ($patterns.Count -eq 0) { return $step }

    # Safely escape characters and join into a single high-performance regex string (e.g. "iex|irm|bypass")
    $regexPattern = ($patterns | ForEach-Object { [regex]::Escape($_) }) -join '|'

    foreach ($userDir in (Get-ChildItem "C:\Users" -Directory -ErrorAction SilentlyContinue)) {
        $hPath = Join-Path $userDir.FullName "AppData\Roaming\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt"

        $file = Get-Item $hPath -ErrorAction SilentlyContinue
        if (-not $file) {
            $step.Data.Add([PSCustomObject]@{ User=$userDir.Name; Command="History missing"; Pattern="N/A"; FileSizeBytes="N/A"; LastWriteTime="N/A" })
            continue
        }

        if ($file.Length -eq 0) {
            $step.Data.Add([PSCustomObject]@{ User=$userDir.Name; Command="Empty history (possible wipe)"; Pattern="Forensic Signal"; FileSizeBytes=0; LastWriteTime=$file.LastWriteTime })
            $step.Data.Add([PSCustomObject]@{ User=$userDir.Name; Command="No suspicious patterns"; Pattern="Status"; FileSizeBytes=0; LastWriteTime=$file.LastWriteTime })
            continue
        }
        ...
        # Use -AllMatches to capture EVERY single keyword that hits on a single line
        $matches = Select-String -LiteralPath $hPath -Pattern $regexPattern -AllMatches -ErrorAction SilentlyContinue

        if ($matches) {
            foreach ($m in $matches) {
                $line = $m.Line.Trim()

                # Extract all unique keywords found in this specific line array
                $matchedKeywords = @($m.Matches.Value | Select-Object -Unique)

                $patternString = $matchedKeywords -join ', '

                $key = "$line|$patternString"
                if (-not $seen[$key]) {
                    $seen[$key] = $true
                    $matchCount++
                    $step.Data.Add([PSCustomObject]@{
        ...
        if ($matchCount -eq 0) {
            $step.Data.Add([PSCustomObject]@{ User=$userDir.Name; Command="No suspicious patterns"; Pattern="Status"; FileSizeBytes=$file.Length; LastWriteTime=$file.LastWriteTime })
        }
    }
    return $step
}
...
function Scan-Step7 {
    $step = New-ScanStep 7 "Registry Autorun Configurations" "Inspects Run/RunOnce keys for suspicious startup entries." "b-orange"
    $rx = ($global:ScanKeywords.AutorunRegistry | ForEach-Object { [regex]::Escape($_) }) -join '|'
    $hives = @(
    ...
    foreach ($h in $hives) {
        if (Test-Path $h) {
            ...
            foreach ($name in ($prop.PSObject.Properties | Where-Object { $_.Name -notmatch "^PS" }).Name) {
                $val = $prop.$name
                if ($val -match $rx) {
                    $step.Issues.Add("Suspicious autorun detected")
                    $step.Data.Add([PSCustomObject]@{ Hive=$h; Name=$name; Value=$val })
                }
            }
        }
    }
    if ($step.Data.Count -eq 0) { $step.Data.Add([PSCustomObject]@{ Hive="All"; Name="Clean"; Value="No suspicious autoruns." }) }
    return $step
}
...
function Scan-Step12 {
    ...
    # 1. Create a temporary array list to hold the raw results prior to sorting
    ...
    foreach ($p in $processes) {
        ...
        # 2. Store the properties cleanly along with the RAW plain-text verdict string
    ...
    # 3. Define a security priority map (Lower number = higher priority / shown first at the top)
    $verdictWeight = @{
        "HIGH RISK USERLAND" = 1
        "INVALID SIGNATURE"  = 2
        "SIGNER MISMATCH"    = 3
        "SUSPICIOUS PATH"    = 4
        "UNVERIFIED"         = 5
    ...
    # 4. Sort the collection by threat weight, then cleanly transform the Verdict string to HTML
    $sortedResults = $tempResults | Sort-Object {
        if ($verdictWeight.ContainsKey($_.RawVerdict)) { $verdictWeight[$_.RawVerdict] } else { 99 }
    }

    # 5. Populate your final telemetry object sequence
    foreach ($item in $sortedResults) {
        $step.Data.Add([PSCustomObject]@{
            IdentityProcess  = $item.IdentityProcess
            FileSystemPath   = $item.FileSystemPath
            DigitalPublisher = $item.DigitalPublisher
            ParentProcess    = $item.ParentProcess
            VerdictStatus    = Get-VerdictHtml $item.RawVerdict
        })
    }
...
# ========================================================================================
# [45] ADVANCED DELETED FILE & EXECUTION TRACE FORENSICS
# ========================================================================================
# PURPOSE:
#   Correlates multiple forensic artifacts to identify deleted executables,
#   cheat remnants, wiped loaders, removed drivers, and suspicious traces.
#
# COLLECTS:
#   - Recycle Bin metadata ($I files)
#   - Broken shortcut targets
#   - Missing Prefetch targets
#   - Missing Startup entries
#   - Missing Run/RunOnce executables
#   - Missing Services/ImagePaths
#   - Suspicious Amcache remnants
#
# NOTES:
#   This is NOT true deleted-file recovery.
#   Windows cannot reliably enumerate deleted files without raw MFT parsing.
#
# ========================================================================================
...
function Scan-Step46 {
    $step = New-ScanStep 46 "Transient Space Execution Targets" "Samples executables from writable locations with signature heuristics." "b-red"
    $cutoff = (Get-Date).AddDays(-30)

    # 1. Safely build the regex pattern from your Global Dictionary
    $namePattern = "update|temp|cache|install|setup" # Fallback default
    if ($global:ScanKeywords -and $global:ScanKeywords.ContainsKey("TransientSpace")) {
        # Joins the array elements into a regex 'or' string: element1|element2|element3
        $namePattern = $global:ScanKeywords["TransientSpace"] -join '|'
    }

    # Using a hash set for O(1) extension lookups
    $exts = [System.Collections.Generic.HashSet[string]]::new([string[]]@(".exe", ".dll", ".ps1", ".bat", ".cmd", ".vbs", ".js", ".hta", ".msi", ".scr"), [System.StringComparer]::OrdinalIgnoreCase)

    # Filter valid root directories
    $roots = @($env:TEMP, $env:TMP, "$env:USERPROFILE\AppData\Local\Temp", "$env:USERPROFILE\Downloads", "$env:USERPROFILE\AppData\Roaming") |
        Where-Object { $_ -and (Test-Path $_ -EA SilentlyContinue) }

    $collectedData = [System.Collections.Generic.List[PSCustomObject]]::new()
    $totalCount = 0

    :rootLoop foreach ($root in $roots) {
        try {
            ...
            $files = [System.IO.Directory]::EnumerateFiles($root, "*", $options)
        } catch { continue }
        ...
        foreach ($filePath in $files) {
            ...
            try {
                ...
                if ($file.LastWriteTime -le $cutoff) { continue }
            } catch { continue }

            # Process file signatures
            $sig = $null
            try { $sig = Get-AuthenticodeSignature $filePath -EA SilentlyContinue } catch {}
            $ss = if ($sig) { $sig.Status } else { "NoSignature" }

            # Optimize flag compilation
            $flags = [System.Collections.Generic.List[string]]::new()
            if ($ss -ne "Valid") { $flags.Add("Unsigned") }
            if ($file.Length -gt 5MB) { $flags.Add("LargeBinary") }

            # 2. Match against your dynamically generated regex pattern
            if ($file.Name -match $namePattern) { $flags.Add("SuspiciousName") }

            $flagCount = $flags.Count
            $level = if ($flagCount -ge 3) { "HIGH" }
                     elseif ($flagCount -eq 2) { "MEDIUM" }
                     elseif ($flagCount -eq 1) { "LOW" }
                     else { "CLEAN" }

            if ($level -eq "HIGH") {
                $step.Issues.Add("HIGH risk binary: $($filePath)")
            }

            $collectedData.Add([PSCustomObject]@{
                FileName  = $file.Name
                SizeKB    = [math]::Round($file.Length / 1KB, 1)
                Signature = $ss
                Flags     = if ($flagCount) { $flags -join "; " } else { "None" }
                Modified  = $file.LastWriteTime
                Verdict   = [string](Get-SuspicionHtml $level)
                RawLevel  = $level
            })

            $totalCount++
            if ($totalCount -ge 200) { break :rootLoop }
        }
    }

    # Sorting logic by Verdict (HIGH -> MEDIUM -> LOW -> CLEAN)
    if ($collectedData.Count -gt 0) {
        $customSortOrder = @{ "HIGH" = 1; "MEDIUM" = 2; "LOW" = 3; "CLEAN" = 4 }
        $sortedData = $collectedData | Sort-Object { $customSortOrder[$_.RawLevel] }

        foreach ($item in $sortedData) {
            $item.PSObject.Properties.Remove("RawLevel")
            $step.Data.Add($item)
        }
    } else {
        $step.Data.Add([PSCustomObject]@{
            FileName  = "No suspicious executables found."
            SizeKB    = 0
            Signature = "N/A"
            Flags     = "None"
            Modified  = "N/A"
            Verdict   = "<span class='v-clean'>[CLEAN]</span>"
        })
    }
